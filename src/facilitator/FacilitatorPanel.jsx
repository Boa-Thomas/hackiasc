...
import {
  ...
  parseTime,
  computePulse,
} from '../admin/facilitatorSchedule'
  ...
  return (
    <div className="min-h-screen bg-dark flex items-center justify-center p-6">
      <div className="card-glass rounded-2xl p-8 text-center max-w-sm">
        <p className="text-hot font-mono text-lg mb-2">Sem acesso</p>
        <p className="text-white/60 text-sm">Abra seu link de facilitador para continuar.</p>
        <a href="#" className="mt-4 inline-block text-electric underline text-sm">Voltar ao início</a>
      </div>
    </div>
  )
}
...
export default function FacilitatorPanel() {
  ...
  // Session guard — check Supabase session and role on mount
  useEffect(() => {
    if (!supabase) { setAuthChecked(true); setHasAccess(false); return }
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) { setAuthChecked(true); setHasAccess(false); return }
  ...
  const loadControls = useCallback(async () => {
    ...
    const [w, s] = await Promise.all([
      supabase.rpc('wall_get_phase'),
      supabase.rpc('get_team_scores_visible'),
    ])
    if (!w.error && w.data != null) setWallPhase(w.data)
  ...
  }, [hasAccess, loadSchedule, loadControls])
  ...
  if (!authChecked) {
    return (
      <div className="min-h-screen bg-dark flex items-center justify-center">
        <p className="text-white/60 font-mono animate-pulse">Verificando acesso…</p>
      </div>
    )
  }
  ...
  if (!supabase) {
    return (
      <div className="min-h-screen bg-dark flex items-center justify-center">
        <p className="text-hot font-mono">Supabase não configurado.</p>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-dark text-white">
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-2xl font-bold text-gradient-cyan">Painel da Facilitadora</h1>
            <p className="text-white/40 text-xs font-mono mt-0.5">HackIA SC 2026 · cockpit ao vivo</p>
          </div>
        </div>
        ...
        {loading ? (
          <p className="text-white/60 font-mono">Carregando cronograma…</p>
        ) : (
          <>
...
function Cronometro({ current, next }) {
  ...
  const remaining = endM != null ? endM - now : null
  const delay = startM != null ? now - startM : null
...
function ScheduleEditor({ days, items, onError, onChanged }) {
  ...
  return (
    <div className="card-glass rounded-2xl p-5">
      ...
      <p className="text-white/40 text-xs mb-4">Arraste pelo ⠿ para reordenar (as setas ↑/↓ também funcionam). Ao mudar um horário, os blocos seguintes do dia podem ser deslocados junto.</p>
      ...
      <div className="space-y-5">
        ...
        {days.length === 0 && <p className="text-white/40 text-sm">Nenhum dia cadastrado.</p>}
      </div>
    </div>
  )
}
