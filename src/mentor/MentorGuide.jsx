...
import {
  VisaoGeral, Metodologia, PapelDoMentor, Cronograma,
  Avaliacao, Ferramentas, Checklist, Glossario, Equipe,
} from './mentorGuideContent'

const SECTIONS = [
  { id: 'visao', label: 'Visão Geral', Component: VisaoGeral },
  { id: 'metodologia', label: 'Metodologia', Component: Metodologia },
  { id: 'papel', label: 'Papel do Mentor', Component: PapelDoMentor },
  { id: 'cronograma', label: 'Cronograma', Component: Cronograma },
  { id: 'avaliacao', label: 'Avaliação', Component: Avaliacao },
  { id: 'ferramentas', label: 'Ferramentas', Component: Ferramentas },
...
export default function MentorGuide({ onBack }) {
      ...
      <header className="sticky top-0 z-20 bg-dark/80 backdrop-blur border-b border-dark-border">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <a href="#mentor" onClick={(e) => { e.preventDefault(); onBack() }} className="font-mono text-lg font-bold tracking-tight">
              <span className="text-cyan">{'>'}</span>
              <span className="text-white">hack</span>
              <span className="text-gradient-cyan">IA</span>
              <span className="text-text-muted">.sc</span>
            </a>
            <span className="hidden sm:inline-block text-text-muted text-xs font-mono uppercase tracking-wider truncate">/ Guia do Mentor</span>
          </div>
          <button
            onClick={onBack}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 pb-3">
          <div className="flex gap-2 overflow-x-auto pb-1">
            {SECTIONS.map((s) => (
              <button
                key={s.id}
                onClick={() => { setActive(s.id); window.scrollTo({ top: 0 }) }}
