...
2. **Reconcile coverage by finding (title + file):** a finding is "covered" only if some returned patch both (a) lists the finding's title in its `bug_refs` AND (b) targets the finding's `file`. Any auto-fixable finding not covered by that title+file match (the fixer crashed, abstained, or fixed only some bugs in a shared file) must NOT vanish — add it to the report-only / needs-review list with a "no safe auto-fix produced" note. (Title-only would mask same-titled findings in different files; file-only would mask a second unfixed bug in a patched file. A same-title-same-file-different-line collision is a rare residual the human resolves from the report.) A security finding never silently disappears.
