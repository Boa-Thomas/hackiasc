...
- **Revoke wiring fix:** the current `revoke(g)` routes only `auth_kind === 'jwt_exchange'` to the edge. Route by `auth_kind` via a `usesEdgeRevoke` helper: `jwt_exchange`/`password` → `access-admin` edge (both have a backing user to delete); `rpc_token` → `admin_revoke_grant` RPC. (Routing by `auth_kind` rather than `supabase_user_id` because `admin_list_grants` does not return `supabase_user_id`; `access-admin` already no-ops the delete when the backing user is absent.)
