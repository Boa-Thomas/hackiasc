-- SP2/B1: re-key mentor/juror/sugar RPCs to session-first dual-mode.
-- Bodies copied verbatim from migrations/phase2_mentor_juror_text.sql;
-- ONLY the identity-resolution preamble is changed per variant:
--   A (juror/mentor): current_X_id() session-first, then token fallback, then RAISE 'unauthorized'
--   B (prepitch): current_mentor_id() session-first, then token fallback, keep existing NULL guard
--   C (sugar_roster): insert session check before p_mentor_token block
-- Resolvers (juror_token_owner, mentor_session_owner, mentor_prepitch_resolve) are NOT re-keyed.
